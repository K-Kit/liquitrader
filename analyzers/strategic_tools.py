...
def verification(data):
    ...
    try:
        ...
        sysprint("Verification error (5)")
        sys.exit(0)

    time_elapsed = time.time() - float(timestamp)
    if time_elapsed > 30.0:
        sysprint("Verification timed out")
        sys.exit(0)
...
def verify(license_key, api_key):
    ...
    try:
        ...
        if b'Web App - Unavailable' in validation_request.content:
            sysprint('You are either being rate limited or something went wrong with the license server.')
            sysprint('If have not received a message about being rate limited, please contact support;')
            sysprint('otherwise, your 10 minutes is not yet up.')
    ...
    if received_verifier_data is None or license_time_remaining is None:
        ...
        if error == 'Invalid request (8)':
            sysprint('Verification error (7)')
        else:
            sysprint(f'Verification error ({"8" if error is None else error})')
    ...
    if received_data is not None:
        if data != received_data:
            sysprint('Verification error (A plea from the devs: we\'ve poured our souls into LiquiTrader;'
